...
/**
 * A control that allows selection of between two Ext.ux.form.MultiSelect
 * controls.
 */
Ext.define('Ext.ux.form.ItemSelector', {
	...
	/**
	 * @cfg {Array} buttons Defines the set of buttons that should be displayed
	 *      in between the ItemSelector fields. Defaults to
	 *      <tt>['top', 'up', 'add', 'remove', 'down', 'bottom']</tt>. These
	 *      names are used to build the button CSS class names, and to look up
	 *      the button text labels in {@link #buttonsText}. This can be
	 *      overridden with a custom Array to change which buttons are displayed
	 *      or their order.
	 */
	buttons : ['top', 'up', 'add', 'remove', 'down', 'bottom'],

	/**
	 * @cfg {Object} buttonsText The tooltips for the {@link #buttons}. Labels
	 *      for buttons.
	 */
	buttonsText : {
		top : "Move to Top",
	...
	initComponent : function() {
		...
		// it copies records from our configured Store into the fromField's
		// Store
		// fyc 2013-6-22注释
		 me.bindStore(me.store);
	},
	// fyc 2013-6-22 添加store参数
	createList : function(title) {
		
		var me = this;
		//Ext.Msg.alert("qqq",store);
		return Ext.create('Ext.ux.form.MultiSelect', {
					// We don't want the multiselects themselves to act like
					// fields,
	 ...
	 * 
	 * @param {Ext.view.BoundList}
	 *            list The list to read selections from.
	...
	getSelections : function(list) {
		...
		return Ext.Array.sort(list.getSelectionModel().getSelection(),
				function(a, b) {
					a = store.indexOf(a);
					b = store.indexOf(b);
	...
	onTopBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list);
	...
	onUpBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list), rec, i = 0, len = selected.length, index = 0;
	...
	onDownBtnClick : function() {
		var list = this.toField.boundList, store = list.getStore(), selected = this
				.getSelections(list), rec, i = selected.length - 1, index = store
				.getCount()
				- 1;
	...
	moveRec : function(add, recs) {
		var me = this, fromField = me.fromField, toField = me.toField, fromStore = add
				? fromField.store
				: toField.store, toStore = add
				? toField.store
				: fromField.store;
	...
	syncValue : function() {
		var me = this;
		me.mixins.field.setValue.call(me, me.setupValue(me.toField.store
						.getRange()));
	},
	...
	setValue : function(value) {
		...
		if (!me.fromStorePopulated) {
			...
			// fyc 2013-06-22 添加,不添加reset无效
			//this.populateFromStore(fromStore);
			return;
		}
